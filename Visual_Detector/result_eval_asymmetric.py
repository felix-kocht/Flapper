...
for i in range(1, int(file_pairs) + 1):
    ...
    output_file = (f'test_cases/case.csv')
    #output_file = (f'test_cases/wide_stiff_{name_string}.csv')

    # Merge the dataframes based on the Time column

    # Perform an outer merge to retain all rows from both DataFrames
    merged_df = pd.merge(df1, df2, on='Time', how='outer')

    # Sort the DataFrame by Time to prepare for interpolation
